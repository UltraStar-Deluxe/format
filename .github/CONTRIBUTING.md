...
If you can't find a suitable issue, you can [open a new issue](https://github.com/ultrastar-deluxe/format/issues/new) or ask us on the [UltraStar discord server](https://discord.gg/tNEXZw2QJX) in the `#song-format` channel.
...
There are several issue templates in this repository that can help to create useful issues.

### Improving The Website

In addition to the formal specification this repository also contains the source code for the [Website](https://usdx.eu/format/) in the `docs` folder.
...
- Potentially feature branches for changes in development.
...
Sometimes changes need to be made independently of the publication process above.
Often these are changes to the [website](https://usdx.eu/format/) in the `docs` folder or changes to the repository structure that have no impact on the contents of the specification.
Non functional changes can be submitted via a PR that directly targets the `main` branch.
The same set of changes should then be merged into the `develop` branch.

> [!WARNING]
> Most changes should follow the standard publication process unless there is a reason not to.
